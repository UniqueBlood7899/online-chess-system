...
public class Board {
		...
		if (game.canPlayerMove()) {
			...
			if (isNextMoveUnlocked) {	
				...
				if (move != null) {
					...
					// For network games, use the network-aware execute method
					if (game.isOnlineGame()) {
						game.executeNetworkMove(move);
					} else {
						executeMove(move);
						endMove();
					}
	...
	public void executeMove(Move move) {
		if (move != null) {
			...
			if (!initializing) {
				// Only render and validate if not during initialization
				render();
				
				// For online games, don't call endMove() here as it will be called by Game.executeNetworkMove
				if (!game.isOnlineGame()) {
					endMove();
				}
			}
